	...
	return func(t *testing.T, context spec.G, it spec.S) {
		...
		context("with no specified version", func() {
			it("builds and runs the app and uses mainline", func() {
				...
				Eventually(logs).Should(ContainSubstring(`No nginx version specified - using mainline => 1.31.`))
		...
		context("with an app specifying mainline", func() {
			it("builds and runs the app", func() {
				...
				Eventually(logs).Should(ContainSubstring(`Requested nginx version: mainline => 1.31.`))
		...
		context("with an app unavailable version", func() {
			it("the build fails and logs and error", func() {
				...
				Eventually(logs).Should(ContainSubstring(`Available versions: mainline, stable, 1.30.x, 1.31.x`))
			})
		})
