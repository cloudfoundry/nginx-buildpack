...
var _ = Describe("CF Nginx Buildpack", func() {
	...
	Context("with included conf files", func() {
		BeforeEach(func() {
			app = cutlass.New(filepath.Join(bpDir, "fixtures", "templated_with_include"))
		})
	...
	Context("with no specified version", func() {
		...
		It("Uses latest mainline nginx", func() {
			...
			Eventually(app.Stdout.String).Should(ContainSubstring(`No nginx version specified - using mainline => 1.21.`))
	...
	Context("with an nginx app specifying mainline", func() {
		...
		It("Logs nginx buildpack version", func() {
			...
			Eventually(app.Stdout.String).Should(ContainSubstring(`Requested nginx version: mainline => 1.21.`))
	...
	Context("with an nginx app specifying stable", func() {
		...
		It("Logs nginx buildpack version", func() {
			...
			Eventually(app.Stdout.String).Should(ContainSubstring(`Requested nginx version: stable => 1.20.`))
	...
	Context("with an nginx app specifying an unknown version", func() {
		...
		It("Logs nginx buildpack versions", func() {
			...
			Eventually(app.Stdout.String).Should(ContainSubstring(`Available versions: mainline, stable, 1.20.x, 1.21.x`))
		})
	})
